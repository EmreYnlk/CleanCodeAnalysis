...
async function showSuggestion(violation) {
    ...
    $("#suggestionBadCode").querySelector("code").textContent  = "Yapay zekadan kod bekleniyor...";
    $("#suggestionGoodCode").querySelector("code").textContent = "Yapay zekadan kod bekleniyor...";
    $("#suggestionPrinciple").innerHTML = "<span class=\"principle-tag\">⏳</span> Gemini API yanıtı bekleniyor...";
    ...
    const data = await generateSuggestion(violation);
...
async function generateSuggestion(violation) {
    let content = "";
    if (state.report && state.report.dosyalar && state.report.dosyalar[violation.file]) {
        content = state.report.dosyalar[violation.file].content || "";
    }
    ...
    try {
        ...
        const data = await fetchJson("/api/v1/suggest", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        return {
            bad: "# API Hatası",
            good: "# Yapay zeka hizmetine erişilemedi.",
            principle: "Lütfen Gemini API anahtarınızı yapılandırdığınızdan emin olun veya konsolu kontrol edin."
        };
    }
}
